...
При компрометации модуля `UCM Master` становится доступна установка модуля в произвольный момент времени, нарушая `ЦБ-2`. Сценарий аналогичен `НС-2`.
Негативные последствия, не приводащие к нарушению целей безопасности аналогичны `НС-3`.
...
| --- |  --- |  --- | 
| `Communication interface` | <span style="background-color:red;color:white">недоверенный</span> | Компрометация не нарушает ЦБ | 
| `UCM Master` | <span style="background-color:red;color:white">недоверенный</span> | Компрометация не нарушает ЦБ при доверенном компоненте `Cryptography` |
| `UCM` | <span style="background-color:green;color:white">доверенный</span> | `НС-1` |
| `OTA Client` | <span style="background-color:red;color:white">недоверенный</span> | не нарушает ЦБ при доверенном компоненте `Cryptography` |
| `State Manager` | <span style="background-color:green;color:white">доверенный</span> | `НС-2` |
| `Cryptography` | <span style="background-color:green;color:white">доверенный</span> | `НС-3`, `НС-6` |
